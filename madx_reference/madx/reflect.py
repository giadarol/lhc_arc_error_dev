...
class Transition:
    ...
    def __init__(self, name, condition, incomingState, outcomingState, action=0, replay=0, consume=True):
        ...
        if incomingState in State.states and outcomingState in State.states:
    ...
    def step(self, context):  # context is currently processed line
        ...
        if self.current == 0:
            ...
            for t in self.current.outputTransitions:
                ...
                if t.accept(context):
                    if t.action != 0:
                        spec = inspect.getfullargspec(t.action)
            ...
            if self.current.final:
                ...
                raise "reached final state before having consumed all lines ("  # had the chance to cross transition to eat-up all end blank lines and comments for instance

            if self.current.action != 0:
                # sys.stderr.write('execute state action\n')
                spec = inspect.getfullargspec(self.current.action)
                args, varargs, varkw, defaults = (
                    spec.args,
                    spec.varargs,
                    spec.varkw,
    ...
    def replay(self):  # go through the pre-recorded trajectory and invoke output instead of input actions
        ...
        for step in self.trajectory:
            if step.__class__.__name__ == 'Transition':
                t = step
                if t.replay != 0:
                    spec = inspect.getfullargspec(t.replay)
                    args, varargs, varkw, defaults = (
                        spec.args,
                        spec.varargs,
                        spec.varkw,
                    ...
                    if len(args) == 0:
                        ...
                        raise "replay function '"

            elif step.__class__.__name__ == 'State':
                s = step
                if s.replay != 0:
                    spec = inspect.getfullargspec(s.replay)
                    args, varargs, varkw, defaults = (
                        spec.args,
                        spec.varargs,
                        spec.varkw,
...
class MathExpr:
    ...
    def tryMatch(str2, lvl=0):  # returns an expression if top-down parsing succeeds, otherwise exception

        if lvl == 0:
            str2 = removeBlanks(str2)

        try:
            parsedExpression = ParenthesisExpr.tryMatch(str2, lvl)
        except MathException:
            try:
                #print "trying to match binary expression for str="+str
                parsedExpression = BinaryExpr.tryMatch(str2, lvl)
                #print "returned?"
            except MathException:
                #print "what about here?"
                try:
                    # unary expressions not used for time-being, instead relying on signs for leaf expressions
                    parsedExpression = UnaryExpr.tryMatch(str2, lvl)
                except MathException:
                    try:
                        parsedExpression = LeafExpr.tryMatch(str2, lvl)
                        #print "leafExpression to be returned"
                    except MathException:
                        #print "do we end-up here in the end?"
                        msg = "failed to match expression '"+str2+"'"
...
#
#    str3 = '-x.D+y-z+12345.678*(b*(c+d))-(e+f-g)'
#    #str = 'pIP1.L1+IP1OFS.B2*DS'
#
#    mathExpr = MathExpr.tryMatch(str3)
...
rootTypes = ('SBEND', 'RBEND', 'DIPEDGE',
             ...
             'ECOLLIMATOR', 'RCOLLIMATOR', 'COLLIMATOR', 'RFCAVITY', 'SOLENOID','CRABCAVITY')
...
class Parameter:
    ...
    def registerParameters(elementName, parameterList):
        ...
        if elementName in Parameter.elementParameters:
...
class Element:  # the base class for all elements that may be affected by a sequence-reflection
    ...
    def __init__(self, name, template, instanceStr, beam=0):
        ...
        if name in Element.elementsDictionary:
            raise ReflectorException(
    ...
    def factory(elementName, elementTemplateName, instanceStr, beam=0):  # static/class method
        ...
        for cls in [Rbend, Sbend,
                    ...
                    RfCavity,
                    Collimator, RCollimator, ECollimator,
                    Marker, PlaceHolder,
                    CrabCavity]:
    ...
    def reflection(self):
        ...
        if self.name not in Parameter.elementParameters:
            raise "error: element "
        ...
        for parameter in parameters:
            if parameter.getName() == paramName:
                ...
                try:
                    ...
                    return True
                except MathException as e:  # from Python 2.6, should end with "as e" instead
...
class Multipole(Element):
    ...
    def reflect(self):
        ...
        try:
            parameters = Parameter.elementParameters[self.name]
        except ssion.MathException as e:  # from Python 2.6, should end with "as e" instead
...
def main():
    ...
    if options.repeat:
        ...
        if not (repeat >= 0 and repeat <= 2):
            raise "expect repeat to be 0, 1 or 2, instead of "
    ...
    if options.sequence or not options.aperture:  # sequence is default mode
        ...
        if options.filename:
            ...
            filename = "/afs/cern.ch/eng/lhc/optics/runIII/lhc_acc-models-lhc.seq"
...
def run(filename, repeat, rename_b2_b4, quietFlag=True, modeFlag='sequence', debugFlag=False, discardB1=False, baseClassesAsImplicitTypes=False):
    ...
    def outputTypeDeclarations(memory):
        ...
        for n in memory:
            ...
            if t.getName() in Parameter.elementParameters:
...
if __name__ == '__main__':
    ...
    try:
        main()
    except ReflectorException as e:  # from Python 2.6, should end with "as e" instead
